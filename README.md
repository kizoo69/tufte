...
- **Marginnote/Sidenote/Figure `<p>`-in-`<span>` Fix**: `figure.html` (`caption`/`title`/`attr`), `marginnote.html`, and `sidenote.html` all run their content through `markdownify`, which wraps it in `<p>...</p>`. That's invalid HTML inside `<span class="marginnote">`/`<span class="sidenote">` — a block-level `<p>` can't legally nest inside an inline `<span>`. Browsers "fix" it by closing the *enclosing* paragraph the moment they hit the inner `<p>`, so the marginnote/sidenote content spills out of the span and lands after whatever block happened to be open at that point (e.g. a `<ul>`), instead of staying attached to its image. This was always broken, in any context — it just went unnoticed when the enclosing text was short enough, or the misplaced fragment landed somewhere visually unremarkable. Nesting inside our own `details` shortcode (which re-parses its `.Inner` via `RenderString`) made it obvious, because the corruption showed up right next to clearly related content. Fixed by stripping `markdownify`'s `<p>`/`</p>` wrapper (single paragraph: `figure.html`) or replacing inter-paragraph `</p><p>` boundaries with `<br><br>` (multi-paragraph: `marginnote.html`, `sidenote.html`) so the span's content is pure inline HTML.
